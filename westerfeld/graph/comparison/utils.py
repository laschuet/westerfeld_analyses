...
def are_graphs_essentially_the_same(g1:nx.Graph, g2:nx.Graph):
    ...
    if(sorted(list(g1.nodes))==sorted(list(g2.nodes))):
        return True
    if(sorted([(sorted(item)) for item in g1.edges])==sorted([(sorted(item)) for item in g2.edges])):
        return True
    return False


def get_shared_nodes(G1: nx.Graph, G2: nx.Graph) -> np.array:
...
def get_shared_edges(G1: nx.Graph, G2: nx.Graph) -> list:
    shared_edges = [list(e) for e in [(sorted(item)) for item in G1.edges] if e in [(sorted(item)) for item in G2.edges]] # sort edges, so e1=(n1,n2) is equal to e2=(n2,n1)
    return shared_edges
...
def find_suitable_edges(G1: nx.Graph, G2: nx.Graph, g: nx.Graph):
    ...
    possible = list(filter(lambda x: x[0] in list(g.nodes) or x[1] in list(g.nodes), possible))
    possible = list(filter(lambda x: sorted(x) not in sort_items_universally(g.edges), possible))
    return possible
...
def expand_graph_by_edge(G:nx.Graph, edge, original_graph: nx.Graph) -> nx.Graph:
    G_copy = G.copy()
    G_copy.add_edge(*edge, positiv_correlation=original_graph.edges[edge]['positiv_correlation'])
    return G_copy


def is_subgraph(G_sub:nx.Graph,G:nx.Graph,):
    if not np.array_equal(np.intersect1d(list(G_sub.nodes), list(G.nodes)), list(G_sub.nodes)):
        return False
    if not np.array_equal(get_shared_edges(G_sub, G), sort_items_universally(list(G_sub.edges))):
        return False
    return True
    ...
    while(len(new_g_structures) != 0):
        ...
        for g_expand in new_g_structures:
            ...
            for e in available_expanded_edges:
                ...
                if(is_subgraph(g_expanded_tilde, G1) and is_subgraph(g_expanded_tilde, G2) and not contains_graph(g_expanded_tilde, new_structs)):
                    if(G1.edges[e]['positiv_correlation'] == G2.edges[e]['positiv_correlation']):
...
def get_graph_metrics(G: nx.Graph, phylums : np.ndarray) -> dict:
    ...
    mean_degree_centrality = np.mean(list(degree_centrality.values())) if degree_centrality else 0

    betweenness_centrality = nx.betweenness_centrality(G)
    mean_betweenness_centrality = np.mean(list(betweenness_centrality.values())) if betweenness_centrality else 0

    phylum_count = pd.DataFrame(pd.DataFrame(data=list(G.nodes.data()))[1].map(lambda it: it['Phylum']).fillna(0).value_counts())
...
def compare_graphs(graphs: list[nx.Graph], names: list[str], phylums : np.ndarray,  out: str = None) -> pd.DataFrame:
    res = pd.DataFrame([get_graph_metrics(g, phylums) for g in graphs], index=["graph_"+t for t in names])
    logging.info("Compare Graphs")
    if(out):
        res.to_csv(out)


def compare_graphs_pairwise_on(graphs:list[nx.Graph], names:list[str], metric:str="nodes_iou", out:str=None, normalized:bool=False):
    ...
    for i, _ in res.iterrows():
        for j, _ in res.iterrows():
            ...
            if(metric == "nodes_iou"):
                ...
                edges_gi = ["|".join(sorted(item)) for item in list(graphs[numeric_i].edges)]
                edges_gj = ["|".join(sorted(item)) for item in list(graphs[numeric_j].edges)]
                ...
                distance = -1
                for edits in nx.optimal_edit_paths(graphs[numeric_i], graphs[numeric_j]):
                    distance =  edits
                res.at[i,j] = distance
            elif(metric == "kernel_shortest_path"):
                res.at[i,j] = kernel_graph([graphs[numeric_i], graphs[numeric_j]], "shortest_path", normalize=normalized)[1,0]
            ...
            #    res.at[i,j] = kernel_graph([graphs[numeric_i], graphs[numeric_j]], "random_walk")[1,0]
                ...
                res.at[i,j] = kernel_graph([graphs[numeric_i], graphs[numeric_j]], "weisfeiler_lehman", normalize=normalized)[1,0]
    if(out):
...
def full_multiple_graphs_evaluation(raw_data: pd.DataFrame, transformed_data: list, seperator: np.ndarray, phylums : np.ndarray, lookup_data: pd.DataFrame):
    ...
    compare_graphs(graphs=graphs, names=seperator,phylums=phylums,  out="out/results.csv")
    compare_graphs_pairwise_on(graphs=graphs, names=seperator, metric="nodes_iou", out="out/nodes_iou_matrics.png")
    compare_graphs_pairwise_on(graphs=graphs, names=seperator, metric="edges_iou", out="out/edges_iou_matrics.png")
    compare_graphs_pairwise_on(graphs=graphs, names=seperator, metric="kernel_shortest_path", out="out/kernel_shortest_path_norm.png", normalized=True)
    ...
    for i in range(len(graphs)):
        for j in range(len(graphs)):
            if(i > j):
                found_patterns = find_similar_subgraphs(G1=graphs[i], G2=graphs[j], n=50)
